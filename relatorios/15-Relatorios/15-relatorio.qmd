...
prop_a <- versaoA_cliques / versaoA_EMAILS
prop_b <- versaoB_cliques / versaoB_emails
...
```{r}
# Diferença observada (B - A)
diferenca_observada <- prop_a - prop_b
diferenca_observada
```
...
Podemos perceber que houve uma diferença de 1.3 pontos percentuais a favor da versao A. 
Recomendação da versao A pois ela é 2.12 pontos percentuais superior a versão B.
